...
LOG_CSV = KB_DIR / "openfda_drug_label_log.csv"
...
def search_openfda(term: str, limit: int) -> list[dict]:
    search = f'openfda.generic_name:"{term}"'
    url = "https://api.fda.gov/drug/label.json?" + urllib.parse.urlencode({"search": search, "limit": limit})
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download openFDA drug label records for seeded medicine terms.")
    parser.add_argument("--max-terms", type=int, default=100, help="Maximum number of generic terms to query.")
    parser.add_argument("--results-per-term", type=int, default=3, help="Max API results per queried term.")
    ...
    for idx, term in enumerate(terms, start=1):
        try:
            results = search_openfda(term, args.results_per_term)
            new_rows = []
            for result in results:
                key = result_key(result)
                if key in seen_keys:
                    continue
                result["_source_term"] = term
    ...
    print(f"OK: openFDA raw data saved to {RAW_JSONL}")
    print(f"OK: openFDA log saved to {LOG_CSV}")
